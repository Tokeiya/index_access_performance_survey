...
def main(package: str, module: str):
    result = subprocess.run(['rustup', 'default'], stdout=subprocess.PIPE, stderr=True, text=True)
    recent_toolchain = result.stdout

    recent_toolchain = re.sub(r"^(.*?)(\(.*)", r'\1', recent_toolchain).strip()
    print(f'recent_toolchain:{recent_toolchain}')

    subprocess.run(['rustup', 'default', 'my-nightly'])
    ...
    subprocess.run(['rustup', 'default', recent_toolchain])
